...
const CustomInput: React.FC<CustomInputProps> = ({
  ...
  return (
    <div className="flex flex-col w-full gap-1.5">
      {/* Label estilizada */}
      {label && (
        <FieldLabel className="text-sm font-bold text-[#851114]">
          {label}
        </FieldLabel>
      )}
      ...
      {type === "select" ? (
        <select
          ...
          className={clsx(
            baseStyles,
            "appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cpath%20d%3D%22M7%2010l5%205%205-5z%22%20fill%3D%22%23666%22%2F%3E%3C%2Fsvg%3E')] bg-no-repeat bg-right pr-8",
          )}
        >
          {children}
        </select>
        ...
        <input
          ...
          className={clsx(
            baseStyles,
            "text-gray-500 file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-brand-primary file:text-brand-primary hover:file:text-white hover:file:bg-[#1e3a8a]",
          )}
        />
      ) : (
        <Input
