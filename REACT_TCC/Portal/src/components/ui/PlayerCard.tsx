...
const PlayerCard: React.FC<PlayerCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "relative w-full h-full transition-transform duration-700 shadow-[8px_8px_0px_1px_#B20000]",
      ...
      >
        {/* === FRENTE DO CARD === */}
        <div
          className="absolute w-full h-full bg-white overflow-hidden border-[10px] border-brand-darkred flex flex-col"
          style={{
        ...
        >
          {/* Badge de Rating */}
          <div className="absolute top-0 left-0 bg-brand-darkred text-white font-bold text-5xl w-20 h-14 flex items-center justify-center rounded-br-2xl z-10 shadow-md">
            {Number(player.rating_medio || 0).toFixed(1)}
          </div>

          {/* Badge de Posição */}
          <div className="absolute bottom-8 right-0 bg-brand-darkred text-white px-3 py-1 text-xl font-bold rounded-l-lg z-10 shadow-md">
        ...
        {/* === VERSO DO CARD === */}
        <div
          className="absolute w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 border-4 border-brand-darkred p-5 flex flex-col justify-between text-brand-primary"
          style={{
            backfaceVisibility: "hidden",
            WebkitBackfaceVisibility: "hidden", // [3] O SEGREDO DO SAFARI
        ...
        >
          {/* Nome no Topo */}
          <div className="bg-brand-darkred text-white -mx-5 -mt-5 p-4 text-center font-bold text-xl mb-2 shadow-sm">
            {player.pessoa?.nome_completo}
          </div>

          {/* Informações */}
          <div className="space-y-3 flex-1 overflow-y-auto pr-1 text-sm font-semibold">
            <div className="flex justify-between border-b border-brand-darkred/30 pb-1">
              <span className="text-brand-darkred">Divisão:</span>
              <span>{player.pessoa?.sub_divisao || "N/A"}</span>
            </div>
            <div className="flex justify-between border-b border-brand-darkred/30 pb-1">
              <span className="text-brand-darkred">Altura:</span>
              <span>{player.altura_cm ? `${player.altura_cm} cm` : "-"}</span>
            </div>
            <div className="flex justify-between border-b border-brand-darkred/30 pb-1">
              <span className="text-brand-darkred">Peso:</span>
              <span>{player.peso_kg ? `${player.peso_kg} kg` : "-"}</span>
            </div>
            <div className="flex justify-between border-b border-brand-darkred/30 pb-1">
              <span className="text-brand-darkred">Pé:</span>
              <span>{player.pe_preferido || "-"}</span>
            </div>

            {/* Última Avaliação */}
            <div className="mt-4">
              <p className="text-brand-darkred underline mb-1">
                Avaliação Recente:
              </p>
              <p className="text-xs text-gray-600 italic leading-relaxed">
                "
                {player.ultima_avaliacao?.observacoes ||
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              onViewMore(player.id);
            }}
            className="w-full mt-4 bg-brand-primary text-white py-2 rounded-lg font-bold hover:bg-[#1e3a8a] transition shadow-md flex items-center justify-center gap-2"
          >
