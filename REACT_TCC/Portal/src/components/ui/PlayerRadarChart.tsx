...
const PlayerRadarChart: React.FC<PlayerRadarChartProps> = ({ avaliacao, posicao }) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
          <p className="font-bold text-[#14244D] dark:text-white">
    ...
    <div className="w-full h-[300px] sm:h-[350px] bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col items-center justify-center">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
          ...
          <PolarAngleAxis dataKey="atributo" tick={{ fill: '#6b7280', fontSize: 12, fontWeight: 'bold' }} />
          <PolarRadiusAxis angle={30} domain={[0, 10]} tick={{ fill: '#9ca3af', fontSize: 10 }} tickCount={6} />
          <Tooltip content={<CustomTooltip />} />
          <Radar name="Jogador" dataKey="nota" stroke="#8B0000" fill="#8B0000" fillOpacity={0.5} dot={{ r: 4, fill: '#14244D' }} activeDot={{ r: 6, fill: '#14244D' }} />
        </RadarChart>
      </ResponsiveContainer>
    </div>
  );
