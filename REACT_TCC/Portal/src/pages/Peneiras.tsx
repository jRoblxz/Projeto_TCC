      ...
      <div className="p-6 space-y-6 min-h-screen pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 dark:bg-gray-900 bg-white p-4 rounded-xl shadow-sm border dark:border-gray-700 border-gray-100">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <button onClick={handleOpenNew} className="flex items-center gap-2 bg-[#14244D] hover:bg-[#1e3a8a] text-gray-200 px-4 py-2 rounded-lg font-semibold transition shadow-md whitespace-nowrap">
              <Plus className="h-5 w-5" /> <span className="hidden sm:inline">Nova Peneira</span>
            </button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {peneiras.length > 0 ? (
                peneiras.map((peneira) => (
                  <div key={peneira.id} className="bg-white dark:bg-gray-900 rounded-xl shadow-sm hover:shadow-lg transition border border-gray-200 dark:border-gray-700  flex flex-col justify-between overflow-hidden group">
                    <div className="p-5 border-b border-gray-100 dark:border-gray-700 ">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="font-bold text-lg text-gray-800 dark:text-gray-200  leading-tight transition">
                          {peneira.title}
                        </h3>
                    ...
                    <div className="p-5 space-y-3 flex-1 text-sm text-gray-600 dark:text-gray-300">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate font-medium text-gray-600 dark:text-gray-300">{peneira.sub_divisao || 'Geral'}</span>
                      </div>
                      {peneira.description && (
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 flex justify-between gap-2 border-t border-gray-100 dark:border-gray-700 ">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div>
                  ...
                  <input type="text" required value={formData.nome_evento} onChange={(e) => setFormData({...formData, nome_evento: e.target.value})} className="w-full dark:bg-gray-800 border-gray-300 dark:border-gray-700  border p-2 rounded outline-none focus:ring-2 focus:ring-[#14244D]" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium mb-1">Data *</label>
                        <input type="datetime-local" required value={formData.data_evento} onChange={(e) => setFormData({...formData, data_evento: e.target.value})} className="w-full bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 border p-2 rounded outline-none focus:ring-2 focus:ring-[#14244D]" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1">Local *</label>
                        <input type="text" required value={formData.local} onChange={(e) => setFormData({...formData, local: e.target.value})} className="w-full bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 border p-2 rounded outline-none focus:ring-2 focus:ring-[#14244D]" />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                        ...
                        <select value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})} className="w-full bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 border p-2 rounded outline-none focus:ring-2 focus:ring-[#14244D] bg-white">
                          <option value="AGENDADA">Agendada</option>
                          <option value="EM_ANDAMENTO">Em Andamento</option>
                          <option value="FINALIZADA">Finalizada</option>
                          <option value="CANCELADA">Cancelada</option>
                ...
                <div>
                  ...
                  <textarea rows={3} value={formData.descricao} onChange={(e) => setFormData({...formData, descricao: e.target.value})} className="w-full bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 border p-2 rounded outline-none focus:ring-2 focus:ring-[#14244D] resize-none" />
                </div>
                <div className="flex justify-end gap-3 pt-4 border-t">
                  <button type="button" onClick={() => setShowFormModal(false)} className="px-4 py-2 bg-gray-100 dark:bg-gray-700 rounded dark:hover:bg-gray-800 hover:bg-gray-200">Cancelar</button>
