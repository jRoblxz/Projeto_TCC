...
import { 
  ArrowLeft, UserPlus, User, Mail, Lock, ShieldCheck, 
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#14244D] dark:text-white flex items-center gap-2">
                <UserPlus className="text-[#8B0000]" /> Novo Usuário
              </h1>
          ...
          <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-8">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <label className={`flex-1 flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all ${formData.role === 'treinador' ? 'border-[#8B0000] bg-red-50 dark:bg-red-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}>
                  <input 
                    type="radio" name="role" value="treinador" 
                  ...
                  <div className="ml-3">
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400">Pode avaliar jogadores e ver relatórios.</span>
                  </div>
                </label>

                <label className={`flex-1 flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all ${formData.role === 'adm' ? 'border-[#14244D] bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}>
                  <input 
                    type="radio" name="role" value="adm" 
                    checked={formData.role === "adm"}
                    onChange={(e) => handleInputChange("role", e.target.value)}
                    className="w-5 h-5 text-[#14244D] focus:ring-[#14244D]" 
                  />
                  <div className="ml-3">
                    <span className="block font-bold text-gray-800 dark:text-gray-200">Administrador</span>
                    <span className="text-xs text-gray-500 dark:text-gray-400">Acesso total às configurações do sistema.</span>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_200px] gap-8">
              ...
              <div className="flex flex-col items-center justify-start">
                ...
                <label className="w-40 h-40 rounded-full border-4 border-dashed border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 cursor-pointer hover:border-[#8B0000] hover:bg-red-50 dark:hover:bg-red-900/10 transition-all overflow-hidden relative group">
                  {fotoPreview ? (
                    <>
            ...
            {formData.role === "treinador" && (
              <div className="pt-6 border-t border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-top-4">
                <h3 className="text-lg font-bold text-[#14244D] dark:text-white mb-6">Informações Profissionais (Opcional)</h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-2 uppercase">Clube / Organização Atual</label>
                    <div className="relative">
                      <Building className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                      <input
                        type="text" value={formData.clube_organizacao} onChange={(e) => handleInputChange("clube_organizacao", e.target.value)}
                        placeholder="Ex: Grêmio Prudente"
                        className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[#8B0000] outline-none transition text-gray-800 dark:text-white"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-2 uppercase">Cargo</label>
                    <div className="relative">
                      <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                      <input
                        type="text" value={formData.cargo} onChange={(e) => handleInputChange("cargo", e.target.value)}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-2 uppercase">Registro CREF</label>
                    <div className="relative">
                      <Hash className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                      <input
                        type="text" value={formData.cref} onChange={(e) => handleInputChange("cref", e.target.value)}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-2 uppercase">Anos de Experiência</label>
                    <div className="relative">
                      <Award className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                      <input
                        type="number" min="0" value={formData.anos_experiencia} onChange={(e) => handleInputChange("anos_experiencia", e.target.value)}
                        placeholder="Ex: 5"
                        className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[#8B0000] outline-none transition text-gray-800 dark:text-white"
                      />
                    </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-2 uppercase">Biografia / Resumo</label>
                  <div className="relative">
                    <FileText className="absolute left-3 top-4 text-gray-400 w-5 h-5" />
                    <textarea
                      rows={4} value={formData.biografia_resumo} onChange={(e) => handleInputChange("biografia_resumo", e.target.value)}
                      placeholder="Fale um pouco sobre a sua trajetória no futebol..."
                      className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-[#8B0000] outline-none transition text-gray-800 dark:text-white resize-none"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="pt-8 border-t border-gray-100 dark:border-gray-800 flex flex-col-reverse md:flex-row justify-end gap-4">
              ...
              <button
                ...
                className="px-8 py-3 font-bold text-white bg-[#14244D] hover:bg-[#1e3a8a] rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
              >
