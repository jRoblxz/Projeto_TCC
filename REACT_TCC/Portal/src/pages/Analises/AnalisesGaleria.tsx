...
const AnalisesGaleria: React.FC = () => {
  ...
  const handleDelete = async (id: number, titulo: string) => {
    if (!window.confirm(`Tem certeza que deseja excluir a análise "${titulo}"? Todos os vídeos e planilhas gerados serão apagados permanentemente.`)) {
      return;
    }
  ...
  const saveTitle = async (id: number) => {
    ...
    try {
      ...
      setJobs(jobs.map(job => job.id === id ? { ...job, titulo: editTitle } : job));
      setEditingId(null);
    } catch (error) {
      console.error(error);
  ...
  const getStatusDisplay = (status: string) => {
    switch (status) {
      case 'done':
        return { color: 'text-green-600 bg-green-100 border-green-200', icon: <CheckCircle size={16} />, text: 'Concluído' };
      case 'processing':
        return { color: 'text-blue-600 bg-blue-100 border-blue-200', icon: <Loader2 size={16} className="animate-spin" />, text: 'Processando (IA)' };
      case 'pending':
        return { color: 'text-yellow-600 bg-yellow-100 border-yellow-200', icon: <Clock size={16} />, text: 'Na Fila' };
      case 'failed':
        return { color: 'text-red-600 bg-red-100 border-red-200', icon: <AlertTriangle size={16} />, text: 'Falha' };
      default:
        return { color: 'text-gray-600 bg-gray-100 border-gray-200', icon: <Clock size={16} />, text: status };
    }
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white dark:bg-gray-900 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-[#14244D] dark:text-white flex items-center gap-2">
              <Video className="text-[#8B0000]" /> Galeria de Análises Táticas
            </h1>
          ...
          <button
            ...
            className="mt-4 md:mt-0 px-6 py-3 bg-[#8B0000] text-white font-bold rounded-xl hover:bg-[#a01519] transition shadow-md flex items-center gap-2"
          >
          ...
          <>
            <div className="grid gap-6 grid-cols-[repeat(auto-fit,minmax(320px,1fr))]">
              {jobs.map((job) => {
                ...
                return (
                  <div key={job.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden flex flex-col hover:shadow-md transition-shadow duration-300">
                    ...
                    >
                      ...
                      {/* Badge de Status */}
                      <div className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 border ${statusInfo.color} shadow-sm backdrop-blur-md bg-white/90 dark:bg-gray-900/90`}>
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      {editingId === job.id ? (
                        <div className="flex items-center gap-2 mb-3">
                          <input 
                            ...
                            className="flex-1 px-3 py-1.5 text-sm border-2 border-[#14244D] rounded-lg outline-none font-bold text-gray-800 dark:text-white dark:bg-gray-800"
                            autoFocus
                          />
                          <button onClick={() => saveTitle(job.id)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-md transition"><Save size={18}/></button>
                          <button onClick={cancelEditing} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-md transition"><X size={18}/></button>
                        </div>
                      ) : (
                        <h3 
                          className="text-lg font-bold text-gray-800 dark:text-white mb-2 line-clamp-2 cursor-pointer hover:text-[#8B0000] transition-colors"
                          onClick={() => navigate(`/tracking/${job.id}`)}
                      ...
                      <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-4 mt-auto">
                        ...
                        {new Date(job.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </div>
                      ...
                      <div className="pt-4 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center gap-2">
                        <button 
                          onClick={() => navigate(`/tracking/${job.id}`)}
                          className="flex-1 py-2 bg-gray-50 dark:bg-gray-800 hover:bg-[#14244D] hover:text-white text-[#14244D] dark:text-gray-300 font-bold rounded-lg transition-colors text-sm"
                        >
                          Abrir Análise
                        </button>
                        <div className="flex gap-1">
                          <button 
                            ...
                            className="p-2 text-gray-500 hover:text-[#14244D] hover:bg-blue-50 dark:hover:bg-gray-800 rounded-lg transition-colors"
                            title="Editar Nome"
                          >
                            <Edit2 size={18} />
                          </button>
                          <button 
                            onClick={() => handleDelete(job.id, job.titulo || job.original_filename)}
            ...
            {jobs.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700">
                ...
                <p className="text-gray-500 font-bold text-lg">Nenhuma análise processada ainda.</p>
                <p className="text-gray-400 text-sm mt-1">Clique em "Nova Análise" para enviar o seu primeiro vídeo.</p>
              </div>
            )}
          </>
        )}
