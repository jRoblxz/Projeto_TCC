...
const UserControl: React.FC = () => {
  ...
  const loadUsers = async (
    ...
    try {
      ...
      if (roleFilter === "candidato" && subFilter)
        params.subdivisao = subFilter;
  ...
  const renderRoleBadge = (role: string) => {
    switch (role) {
      case "adm":
        return (
          <span className="flex items-center gap-1 w-fit px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-bold border border-blue-200">
            <Shield size={12} /> Admin
          </span>
        );
      case "treinador":
        return (
          <span className="flex items-center gap-1 w-fit px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-bold border border-purple-200">
            <Dumbbell size={12} /> Treinador
          </span>
        );
      case "candidato":
        return (
          <span className="flex items-center gap-1 w-fit px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-bold border border-green-200">
            <UserIcon size={12} /> Jogador
          </span>
        );
      default:
        return (
          <span className="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-bold border border-gray-200">
            {role}
          </span>
        );
    }
        ...
        <div className="flex items-center gap-3 py-2">
          <div className="w-10 h-10 rounded-full border-2 border-gray-200 overflow-hidden flex-shrink-0 bg-gray-100">
            {row.pessoa?.foto_url_completa ? (
              <img
                src={row.pessoa.foto_url_completa}
                alt={row.name}
                className="w-full h-full object-cover"
              />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-gray-400 font-bold bg-brand-primary/5">
          ...
          <div>
            <p className="font-bold text-gray-800 dark:text-white text-sm">
              {row.pessoa?.nome_completo || row.name}
            </p>
      ...
      cell: (row) => (
        <div className="flex flex-col gap-1 py-2">
          {row.role === "candidato" && row.pessoa?.sub_divisao && (
            <span className="text-xs font-bold text-brand-darkred">
              {row.pessoa.sub_divisao}
            </span>
          )}
          {row.role === "candidato" &&
            row.pessoa?.jogador?.posicao_principal && (
              <span className="text-xs text-gray-500">
                Pos. {row.pessoa.jogador.posicao_principal}
              </span>
            )}
          {row.role === "treinador" && row.pessoa?.treinador?.cargo && (
            <span className="text-xs font-medium text-gray-600 dark:text-gray-400">
              {row.pessoa.treinador.cargo}
            </span>
          )}
          {row.role === "adm" && (
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 gap-4">
          <div>
            <h1 className="text-2xl font-bold text-brand-primary dark:text-white flex items-center gap-2">
              <Users className="text-brand-darkred" /> Gestão de Usuários
            </h1>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <button
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-brand-primary text-white font-bold rounded-xl hover:bg-[#1e3a8a] transition shadow-md"
            >
            ...
            {roles.map((r) => (
              <button
                ...
                className={`flex-1 lg:flex-none px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${
                  roleFilter === r.value
                    ? "bg-white dark:bg-gray-700 shadow-sm text-brand-primary dark:text-white"
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto flex-1 justify-end">
            ...
            {roleFilter === "candidato" && (
              <div className="flex items-center bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 animate-in fade-in zoom-in">
                <select
                  ...
                  className="bg-transparent border-none outline-none text-sm font-bold text-brand-darkred cursor-pointer py-2.5 w-full"
                >
            ...
            <div className="relative min-w-[250px]">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-brand-primary outline-none transition text-sm"
              />
            </div>
          </div>
        </div>
          ...
          <DataTable
            ...
            progressComponent={
              <div className="p-10 flex flex-col items-center justify-center">
                <div className="w-10 h-10 border-4 border-t-brand-primary border-gray-200 rounded-full animate-spin mb-4"></div>
