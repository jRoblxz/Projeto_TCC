...
import PlayerHeader from "@/components/players/View/PlayerHeader";
import PlayerStats from "@/components/players/View/PlayerStats";
import PlayerDetails from "@/components/players/View/PlayerDetails";
import PlayerDeleteButton from "@/components/players/View/PlayerDeleteButton";
...
const PlayerInfo: React.FC = () => {
  ...
  const getDisplayRating = () => {
    ...
    if (player.rating_medio && Number(player.rating_medio) > 0)
      return Number(player.rating_medio).toFixed(1);
    if (player.rating_calculado)
      return Number(player.rating_calculado).toFixed(1);
    return "0.0";
  };
  ...
  return (
    <Layout>
      <div className="max-w-[1400px] mx-auto p-5">
        {/* ========== COMPONENTE HEADER ========== */}
        <PlayerHeader onBack={() => navigate(-1)} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-8 p-4">
          ...
          <PlayerStats
            ...
            onEdit={() => navigate(`/jogadores/${id}/edit`)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-8 p-4">
          <div className="bg-[#f8f9fa] dark:bg-gray-900 rounded-[15px] p-6 shadow-[0_5px_20px_rgba(0,0,0,0.05)] relative">
            ...
            <div className="w-[150px] h-[150px] rounded-full mx-auto mb-5 relative overflow-hidden bg-black/70 border-4 border-white shadow-sm">
              <img
                src={
                  player.pessoa?.foto_url_completa || "/img/avatar_padrao.png"
                }
                alt="Foto"
            ...
            {canViewDetails && (
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                  {getAttributesByPosition(player.posicao_principal).map(
                    (attr) => (
                      <div
                        key={attr.key}
                      ...
                      >
                        ...
                        <div className="text-xl lg:text-2xl text-[#851114] font-bold">
                          ...
                          player.ultima_avaliacao?.[attr.key] !== null
                            ? Number(player.ultima_avaliacao[attr.key]).toFixed(
                                1,
                              )
                            : "N/A"}
                        </div>
