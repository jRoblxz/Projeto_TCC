  ...
  return (
    <Layout>
      <div className="max-w-[1400px] mx-auto p-5">
        ...
        <div className="bg-[#14244D] p-8 text-white text-center mb-5 rounded-none md:rounded-t-lg relative shadow-md">
            <button 
                onClick={() => navigate(-1)} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-8 p-4">
          ...
          {/* COLUNA ESQUERDA */}
          <div className="bg-[#f8f9fa] rounded-[15px] p-6 shadow-[0_5px_20px_rgba(0,0,0,0.05)] relative">
            <button 
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
              {[
                ...
                <div key={i} className="bg-white p-3 rounded-lg text-center shadow-sm border-2 border-[#e9ecef] hover:-translate-y-0.5 hover:shadow-md hover:border-[#851114] transition-all">
                    <div className="text-sm text-gray-500   mb-1">{stat.label}</div>
                    <div className="text-lg font-bold text-[#333]  ">{stat.value}</div>
                </div>
              ))}
              
              <div className="bg-white p-3 rounded-lg text-center shadow-sm border-2 border-[#e9ecef] hover:-translate-y-0.5 transition-all">
                  <div className="text-sm text-gray-500   mb-1">Posição Principal</div>
                  <div className="text-lg font-bold text-[#ff4757]  ">{player.posicao_principal || "N/A"}</div>
              </div>
              <div className="bg-white p-3 rounded-lg text-center shadow-sm border-2 border-[#e9ecef] hover:-translate-y-0.5 transition-all">
                  <div className="text-sm text-gray-500   mb-1">Posição Secundária</div>
          ...
          {/* COLUNA DIREITA */}
          <div className="bg-[#f8f9fa] rounded-[15px] p-6 shadow-[0_5px_20px_rgba(0,0,0,0.05)] flex flex-col gap-6">
            ...
            <div className="bg-white p-5 rounded-[10px] border-l-[5px] border-[#14244D] shadow-sm">
              <h4 className="text-[#333] text-lg font-bold mb-4  ">Informações e Notas</h4>
              <div className="text-[#666] leading-relaxed space-y-2 text-sm md:text-base">
              ...
              <div className="mt-6 pt-4 border-t border-gray-100">
                  <h4 className="text-[#333] text-lg font-bold mb-2  ">Avaliação Recente</h4>
                  <p className="text-[#666] italic bg-gray-50 p-3 rounded">"{player.ultima_avaliacao?.observacoes ?? 'Nenhuma observação.'}"</p>
              </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
               {['Técnica', 'Condicionamento', 'Finalização', 'Velocidade', 'Posicionamento', 'Cabeceio'].map((item) => (
                   <div key={item} className="bg-white p-4 rounded-[10px] text-center shadow-sm border-2 border-[#e9ecef] hover:-translate-y-0.5 hover:border-[#851114] transition-all">
                       <h5 className="text-[#333] mb-2 font-bold text-lg  ">{item}</h5>
